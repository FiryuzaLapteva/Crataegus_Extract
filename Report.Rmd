...
# 6. АНАЛИЗ БЕЗОПАСНОСТИ: КОЛИЧЕСТВЕННЫЕ ПЕРЕМЕННЫЕ

## 6.1 Проверка предварительных условий

### Тест нормальности (Shapiro-Wilk)

```{r shapiro_test, echo=TRUE}
cat("=== ПРОВЕРКА НОРМАЛЬНОСТИ РАСПРЕДЕЛЕНИЯ ===\n")
cat("H0: данные нормально распределены (p > 0.05 ✓)\n\n")

variables_to_check <- c(
  "V1_BIO_CREAT", "V6_BIO_CREAT", "V8_BIO_CREAT",
  "V1_BIO_EGFR", "V6_BIO_EGFR", "V8_BIO_EGFR",
  "V1_BIO_K", "V6_BIO_K","V8_BIO_K"
)

shapiro_results <- data.frame()

for (var in variables_to_check) {
  if (var %in% names(team1)) {
    data_clean <- team1[[var]][!is.na(team1[[var]])]
    if (length(data_clean) > 0) {
      shapiro_test <- shapiro.test(data_clean)
      shapiro_results <- bind_rows(shapiro_results, 
        data.frame(
          Variable = var,
          N = length(data_clean),
...
shapiro_table <- shapiro_results %>%
  ...
  autofit() %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  set_caption("Результаты тестов нормальности (Shapiro-Wilk)")
...
cat("H0: дисперсии равны в группах (p > 0.05 ✓)\n\n")

levene_results <- data.frame()

for (var in c("V1_BIO_CREAT", "V6_BIO_CREAT", "V8_BIO_CREAT",
              "V1_BIO_EGFR", "V6_BIO_EGFR", "V8_BIO_EGFR",
              "V1_BIO_K", "V6_BIO_K","V8_BIO_K")) {
  if (var %in% names(team1)) {
    df_clean <- team1 %>% select(ARM, !!var) %>% filter(!is.na(.data[[var]]))
    if (nrow(df_clean) > 0) {
      levene_test <- car::leveneTest(df_clean[[var]], df_clean$ARM)
      levene_results <- bind_rows(levene_results,
        data.frame(
          Variable = var,
          Statistic = round(levene_test$`F value`[1], 4),
          P_value = round(levene_test$`Pr(>F)`[1], 4),
          Equal_Variances = ifelse(levene_test$`Pr(>F)`[1] > 0.05, "YES ✓", "NO ✗")
        )
      )
    }
  }
...
levene_table <- levene_results %>%
  ...
  autofit() %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  set_caption("Тесты гомогенности дисперсий (Levene's test)")
...
## 6.2 Описательная статистика - Baseline (V1)

```{r descriptive_v1, echo=TRUE}
desc_v1 <- team1 %>%
  select(ARM, V1_BIO_CREAT, V1_BIO_EGFR, V1_BIO_K) %>%
  filter(!is.na(V1_BIO_CREAT) | !is.na(V1_BIO_EGFR) | !is.na(V1_BIO_EGFR)) %>%
  pivot_longer(
    cols = -ARM,
    names_to = "Parameter",
    values_to = "Value"
  ...
  mutate(
    Parameter = recode(Parameter,
                       V1_BIO_CREAT = "Creatinine (mg/dL)",
                       V1_BIO_EGFR = "eGFR (mL/min)",
                       V1_BIO_K = "POTASSIUM (mmol/L)"
                       )
  )

desc_v1_table <- desc_v1 %>%
  flextable() %>%
  theme_booktabs() %>%
  autofit() %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  set_caption("Описательная статистика - Baseline (V1)")
...
## 6.3 ANCOVA анализ: V6 скорректировано на V1

```{r ancova_v6, echo=TRUE}
cat("\n=== ANCOVA: V6 СКОРРЕКТИРОВАНО НА V1 ===\n\n")
...
ANCOVA анализ показал отсутствие статистически значимых различий между группами Crataegus и Placebo по показателям функции почек (креатинин и eGFR) и уровню электролитов (калий) на всех визитах. Креатинин существенно не отличался (V6: p=0.437; V8: p=0.099), eGFR показал аналогичный результат (V6: p=0.119; V8: p=0.311). Уровень калия был идентичен между группами (V6: p=0.970; V8: p=0.323). Таким образом, препарат Crataegus продемонстрировал профиль безопасности, сопоставимый с плацебо, без явных признаков нефротоксичности или электролитного дисбаланса.
---
