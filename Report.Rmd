...
Анализ отдельных компонентов с использованием моделей конкурирующих рисков подтвердил результаты первичного анализа. Высокая частота событий в исследуемой популяции (более половины пациентов были госпитализированы и почти четверть скончались от СС-причин в течение 2 лет) подчеркивает тяжесть отобранной когорты, однако добавление боярышника к оптимальной базовой терапии не оказало влияния ни на риск госпитализаций, ни на показатели смертности.

# Для смерти и госпитализации по отдельности графики CIF
...
Таблица 4.4. Динамика качества жизни по шкале KCCQ
```{r}
# helpers 
...
### Сводная таблица ANCOVA результатов
...
ANCOVA анализ показал отсутствие статистически значимых различий между группами Crataegus и Placebo по показателям функции почек (креатинин и eGFR) и уровню электролитов (калий) на всех визитах. Креатинин существенно не отличался (12 мес: p=0.437; 24 мес: p=0.099), eGFR показал аналогичный результат (12 мес: p=0.119; 24 мес: p=0.311). Уровень калия был идентичен между группами (12 мес: p=0.970; 24 мес: p=0.323). Таким образом, препарат Crataegus продемонстрировал профиль безопасности, сопоставимый с плацебо, без явных признаков нефротоксичности или электролитного дисбаланса.
---
