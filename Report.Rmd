...
tbl_baseline <-
  ...
  tbl_summary(
    ...
    statistic = list(
      ...
      c(V1_BIO_NTPROBNP, V1_KCCQ_OS) ~ "{median} [{p25}, {p75}]", # ЭКГ и витальные показатели
      all_categorical() ~ "{n} ({p})"
    ),
    digits = list(
      all_continuous() ~ 1,
...
dat_primary <- team1 %>%
  transmute(
    usubjid = USUBJID,
    arm = factor(ARM, levels = c("Placebo", "Crataegus")),
    time_m = V0_END_TTE_COMP_M,
    event  = as.integer(V0_END_COMP_EVENT == 1)
  ) %>%
  filter(!is.na(time_m), !is.na(event), !is.na(arm))

# Log-rank (как в DAPA-HF рядом с KM)
lr <- survdiff(Surv(time_m, event) ~ arm, data = dat_primary)

# Cox (основной эффект: HR)
fit_cox <- coxph(Surv(time_m, event) ~ arm, data = dat_primary)
summary(fit_cox)
```
```{r km_primary_event, fig.width=7.5, fig.height=6.5, dpi=300}
...
nejm_theme <- theme_classic(base_size = 12) +
  theme(
    legend.position = "top",
