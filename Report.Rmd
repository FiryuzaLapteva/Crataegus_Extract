...
```
Таблица Х Динамика качества жизни по шкале KCCQ (Клинически значимое улучшение качества жизни по KCCQ)
...
ggplot(dat_bnp, aes(x = BNP0)) +
  ...
  theme_minimal()
```
Рисунок X. Распределение значений NT-proBNP у пациентов на исходном визите
...
ggplot(dat_bnp, aes(x = lBNP0)) +
  ...
  theme_minimal()
```
Рисунок Y. Плотность распределения логарифмированных значений NT-proBNP на исходном визите
...
ggplot(bnp_sum, aes(x = VISIT, y = mean, group = arm, colour = arm)) +
  ...
  theme_minimal(base_size = 12)
```
 Рисунок X. Динамика уровня NT-proBNP в группах лечения
