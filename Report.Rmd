...
При оценке вторичной конечной точки анализ выживаемости не выявил статистически значимых различий между группой экстракта боярышника и группой плацебо. Время до "Смерти ИЛИ Госпитализации" (Primary Composite Endpoint). Здесь используется обычный Каплан-Мейер.
...
# 1. Подготовка данных: статус должен быть фактором
# 0 = Цензурирован, 1 = Госпитализация, 2 = Смерть
data_cif <- data_surv %>%
  mutate(status_factor = case_when(
    type == "CENSORED" ~ 0,
    type == "HF_HOSP"  ~ 1,
    type == "CV_DEATH" ~ 2
  ) %>% factor(levels = 0:2, labels = c("censor", "hosp", "death")))

# 2. Расчет модели Кумулятивной Инцидентности
# Мы оцениваем риск госпитализации, учитывая смерть как конкурирующее событие
cif_model <- cuminc(Surv(Time, status_factor) ~ group, data = data_cif)

# 3. Построение графика по стандартам публикации
plot_cif <- cif_model %>%
  ggcuminc(outcome = "hosp") + 
  add_risktable(
    risktable_stats = c("n.risk", "cum.event")
  )  +             
  add_pvalue(
    caption = "Gray's test {p.value}",
    location = "annotation", # Позволяет разместить внутри графика
    x = 22,                   # Координата по оси X (начало графика)
    label = "Gray's test p={p.value}", # Формат текста
    size = 4                 # Размер шрифта (опционально)
  ) +             
  add_confidence_interval() +   
  scale_ggsurvfit(
    x_scales = list(
      breaks = seq(0, 24, by = 6), # Шаг 6 месяцев
      limits = c(0, 24)            # Обрезать строго на 24
    ),
    y_scales = list(
      labels = scales::percent_format() # Сделать ось Y в процентах (опционально)
    )
  ) +
  labs(
    title = "Cumulative Incidence of HF Hospitalization",
    subtitle = "Accounting for CV death as a competing risk",
    x = "Months",
  ...
  theme_minimal() +
  theme(
    legend.position = c(0.02, 0.95),      # Координаты X (0.02) и Y (0.95)
    legend.justification = c(0, 1),      # Точка привязки (левый верхний угол легенды)
    legend.background = element_rect(fill = "white", color = "grey80"), # Белый фон и рамка, чтобы сетка не мешала
...
```{r fig.height=8, fig.width=10}
plot_death_cif <- cif_model %>%
  ggcuminc(outcome = "death") + # Указываем "death" (уровень 2 в факторе)
  add_risktable(
    risktable_stats = c("n.risk", "cum.event")
  ) +             
  add_pvalue(
  ...
  labs(
    title = "Cumulative Incidence of Cardiovascular Death",
    subtitle = "Accounting for HF hospitalization as a competing risk",
    x = "Months",
    y = "Cumulative Incidence (Probability of Death)"
  ) +
  scale_color_brewer(palette = "Set1") +
  theme_minimal() +
  theme(
...
```{r}
ggsave(
  filename = "cif_hosp_high_res.png", # Имя файла
  plot = plot_cif,                  # Объект графика
  width = 10,                       # Ширина в дюймах
  height = 7,                       # Высота в дюймах
  dpi = 300,                        # Разрешение (300 для презы - идеально)
  bg = "white"                      # Белый фон (важно для PPT, чтобы не был прозрачным)
)

# Сохранение второго графика (Смерть) - замените plot_death на ваш объект
ggsave(
  filename = "cif_death_high_res.png",
  plot = plot_death_cif,
  width = 10,
  height = 7,
  dpi = 300,
...
Таблица Динамика качества жизни по шкале KCCQ
...
Динамика качества жизни по шкале KCCQ (Клинически значимое улучшение качества жизни по KCCQ)
...
Распределение значений NT-proBNP у пациентов на исходном визите
...
Плотность распределения логарифмированных значений NT-proBNP на исходном визите
...
bnp_results <- bind_rows(res12, res24) %>%
  mutate(
    ratio_ci = sprintf("%.2f (%.2f; %.2f)", ratio, lcl, ucl),
    pct_change = sprintf("%+.1f%%", (ratio - 1) * 100),
...
Динамика уровня NT-proBNP в группах лечения
...
В связи с отсутствием в базе данных информации по нежелательным явлениям анализ безопасности был проведен по количественным переменным -  уровню креатинина,  уровню калия в анализах крови и скорости клубочковой фильтрации

```{r ancova_v6,include=FALSE, echo=TRUE}

# ====== ANCOVA для Креатинина V6 ======
df_ancova_creat_v6 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_CREAT, V6_BIO_CREAT) %>%
  filter(!is.na(V1_BIO_CREAT) & !is.na(V6_BIO_CREAT))
# cat("Sample size - Creatinine V6:\n")
# print(table(df_ancova_creat_v6$ARM))

ancova_model_creat_v6 <- lm(
  V6_BIO_CREAT ~ V1_BIO_CREAT + ARM,
  data = df_ancova_creat_v6
)



coef_arm <- coef(ancova_model_creat_v6)["ARMPlacebo"]
se_arm <- summary(ancova_model_creat_v6)$coefficients["ARMPlacebo", "Std. Error"]
t_stat <- summary(ancova_model_creat_v6)$coefficients["ARMPlacebo", "t value"]
p_val <- summary(ancova_model_creat_v6)$coefficients["ARMPlacebo", "Pr(>|t|)"]
ci_lower <- coef_arm - 1.96 * se_arm
ci_upper <- coef_arm + 1.96 * se_arm


df_ancova_egfr_v6 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_EGFR, V6_BIO_EGFR) %>%
  filter(!is.na(V1_BIO_EGFR) & !is.na(V6_BIO_EGFR))
...
df_ancova_egfr_v6 <- team1 %>%
  ...
  filter(!is.na(V1_BIO_EGFR) & !is.na(V6_BIO_EGFR))
# cat("\nSample size - eGFR V6:\n")
# print(table(df_ancova_egfr_v6$ARM))

ancova_model_egfr_v6 <- lm(
  V6_BIO_EGFR ~ V1_BIO_EGFR + ARM,
  data = df_ancova_egfr_v6
)


coef_arm_egfr <- coef(ancova_model_egfr_v6)["ARMPlacebo"]
se_arm_egfr <- summary(ancova_model_egfr_v6)$coefficients["ARMPlacebo", "Std. Error"]
t_stat_egfr <- summary(ancova_model_egfr_v6)$coefficients["ARMPlacebo", "t value"]
p_val_egfr <- summary(ancova_model_egfr_v6)$coefficients["ARMPlacebo", "Pr(>|t|)"]
ci_lower_egfr <- coef_arm_egfr - 1.96 * se_arm_egfr
ci_upper_egfr <- coef_arm_egfr + 1.96 * se_arm_egfr




df_ancova_K_v6 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_K, V6_BIO_K) %>%
  filter(!is.na(V1_BIO_K) & !is.na(V6_BIO_K))
...
df_ancova_K_v6 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_K, V6_BIO_K) %>%
  filter(!is.na(V1_BIO_K) & !is.na(V6_BIO_K))
# cat("Sample size - POTASSIUM V6:\n")
# print(table(df_ancova_K_v6$ARM))

ancova_model_K_v6 <- lm(
  V6_BIO_K ~ V1_BIO_K + ARM,
  data = df_ancova_K_v6
)



coef_arm_K <- coef(ancova_model_K_v6)["ARMPlacebo"]
se_arm_K<- summary(ancova_model_K_v6)$coefficients["ARMPlacebo", "Std. Error"]
t_stat_K <- summary(ancova_model_K_v6)$coefficients["ARMPlacebo", "t value"]
p_val_K <- summary(ancova_model_K_v6)$coefficients["ARMPlacebo", "Pr(>|t|)"]
ci_lower_K <- coef_arm - 1.96 * se_arm
ci_upper_K <- coef_arm + 1.96 * se_arm



df_ancova_creat_v8 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_CREAT, V8_BIO_CREAT) %>%
  filter(!is.na(V1_BIO_CREAT) & !is.na(V8_BIO_CREAT))


# ====== ANCOVA для Креатинина V8 ======
df_ancova_creat_v8 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_CREAT, V8_BIO_CREAT) %>%
  filter(!is.na(V1_BIO_CREAT) & !is.na(V8_BIO_CREAT))
# cat("\nSample size - Creatinine V8:\n")
# print(table(df_ancova_creat_v8$ARM))

ancova_model_creat_v8 <- lm(
  V8_BIO_CREAT ~ V1_BIO_CREAT + ARM,
  data = df_ancova_creat_v8
)

coef_arm_v8 <- coef(ancova_model_creat_v8)["ARMPlacebo"]
se_arm_v8 <- summary(ancova_model_creat_v8)$coefficients["ARMPlacebo", "Std. Error"]
t_stat_v8 <- summary(ancova_model_creat_v8)$coefficients["ARMPlacebo", "t value"]
p_val_v8 <- summary(ancova_model_creat_v8)$coefficients["ARMPlacebo", "Pr(>|t|)"]
ci_lower_v8 <- coef_arm_v8 - 1.96 * se_arm_v8
ci_upper_v8 <- coef_arm_v8 + 1.96 * se_arm_v8


df_ancova_egfr_v8 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_EGFR, V8_BIO_EGFR) %>%
  filter(!is.na(V1_BIO_EGFR) & !is.na(V8_BIO_EGFR))
...
df_ancova_egfr_v8 <- team1 %>%
  ...
  filter(!is.na(V1_BIO_EGFR) & !is.na(V8_BIO_EGFR))
# cat("\nSample size - eGFR V8:\n")
# print(table(df_ancova_egfr_v8$ARM))

ancova_model_egfr_v8 <- lm(
  V8_BIO_EGFR ~ V1_BIO_EGFR + ARM,
  data = df_ancova_egfr_v8
)



coef_arm_egfr_v8 <- coef(ancova_model_egfr_v8)["ARMPlacebo"]
se_arm_egfr_v8 <- summary(ancova_model_egfr_v8)$coefficients["ARMPlacebo", "Std. Error"]
t_stat_egfr_v8 <- summary(ancova_model_egfr_v8)$coefficients["ARMPlacebo", "t value"]
p_val_egfr_v8 <- summary(ancova_model_egfr_v8)$coefficients["ARMPlacebo", "Pr(>|t|)"]
ci_lower_egfr_v8 <- coef_arm_egfr_v8 - 1.96 * se_arm_egfr_v8
ci_upper_egfr_v8 <- coef_arm_egfr_v8 + 1.96 * se_arm_egfr_v8


df_ancova_K_v8 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_K, V8_BIO_K) %>%
  filter(!is.na(V1_BIO_K) & !is.na(V8_BIO_K))
...
df_ancova_K_v8 <- team1 %>%
  select(USUBJID, ARM, V1_BIO_K, V8_BIO_K) %>%
  filter(!is.na(V1_BIO_K) & !is.na(V8_BIO_K))
# cat("\nSample size - POTASSIUM V8:\n")
# print(table(df_ancova_K_v8$ARM))

ancova_model_K_v8 <- lm(
  V8_BIO_K ~ V1_BIO_K + ARM,
  data = df_ancova_K_v8
)

coef_arm_K_v8 <- coef(ancova_model_K_v8)["ARMPlacebo"]
se_arm_K_v8 <- summary(ancova_model_K_v8)$coefficients["ARMPlacebo", "Std. Error"]
t_stat_K_v8 <- summary(ancova_model_K_v8)$coefficients["ARMPlacebo", "t value"]
p_val_K_v8 <- summary(ancova_model_K_v8)$coefficients["ARMPlacebo", "Pr(>|t|)"]
ci_lower_K_v8 <- coef_arm_v8 - 1.96 * se_arm_v8
ci_upper_K_v8 <- coef_arm_v8 + 1.96 * se_arm_v8

```

### Сводная таблица ANCOVA результатов безопасности

```{r}

summary_results <- data.frame(
  Parameter = c("Creatinine", "Creatinine", "eGFR", "eGFR","Potassium","Potassium" ),
  Visit = c("V6", "V8", "V6", "V8","V6", "V8"),
  Group_Difference = c(
    round(coef_arm, 3),
    round(coef_arm_v8, 3),
    round(coef_arm_egfr, 2),
    round(coef_arm_egfr_v8, 2),
    round(coef_arm_K, 2),
    round(coef_arm_K_v8, 2)
  ),
  CI_Lower = c(
    round(ci_lower, 3),
    round(ci_lower_v8, 3),
    round(ci_lower_egfr, 2),
    round(ci_lower_egfr_v8, 2),
    round(ci_lower_K, 2),
    round(ci_lower_K_v8, 2)
  ),
  CI_Upper = c(
    round(ci_upper, 3),
    round(ci_upper_v8, 3),
    round(ci_upper_egfr, 2),
    round(ci_upper_egfr_v8, 2),
    round(ci_upper_K, 2),
    round(ci_upper_K_v8, 2)
  ),
  t_stat = c(
    round(t_stat, 3),
    round(t_stat_v8, 3),
    round(t_stat_egfr, 3),
    round(t_stat_egfr_v8, 3),
    round(t_stat_K, 3),
    round(t_stat_K_v8, 3)
  ),
  p_value = c(
    ...
    round(p_val_v8, 4),
    round(p_val_egfr, 4),
    round(p_val_egfr_v8, 4),
    round(p_val_K, 4),
    round(p_val_K_v8, 4)),
  Significant = c(
    ifelse(p_val < 0.05, "YES ★", "NO"),
    ifelse(p_val_v8 < 0.05, "YES ★", "NO"),
    ifelse(p_val_egfr < 0.05, "YES ★", "NO"),
    ifelse(p_val_egfr_v8 < 0.05, "YES ★", "NO"),
    ifelse(p_val_K < 0.05, "YES ★", "NO"),
    ifelse(p_val_K_v8 < 0.05, "YES ★", "NO")
  )
)

ancova_summary_table <- summary_results %>%
  flextable() %>%
  theme_booktabs() %>%
  autofit() %>%
  align(j = 2:8, align = "center", part = "all") %>%
  set_table_properties(layout = "autofit", width = 1) %>%
  set_caption("ANCOVA анализ - Сравнение групп (Placebo vs Crataegus, adjusted for V1)")
  
ancova_summary_table
```

ANCOVA анализ показал отсутствие статистически значимых различий между группами Crataegus и Placebo по показателям функции почек (креатинин и eGFR) и уровню электролитов (калий) на всех визитах. Креатинин существенно не отличался (12 месяцев: p=0.437; 24 месяца: p=0.099), eGFR показал аналогичный результат (12 месяцев: p=0.119; 24 месяца: p=0.311). Уровень калия был идентичен между группами (12 месяцев: p=0.970; 24 месяца: p=0.323). Таким образом, препарат Crataegus продемонстрировал профиль безопасности, сопоставимый с плацебо, без явных признаков нефротоксичности или электролитного дисбаланса.

# Заключение

Проведенный анализ не продемонстрировал статистически значимого превосходства экстракта боярышника (Crataegus) по сравнению с плацебо в популяции пациентов с хронической сердечной недостаточностью со сниженной фракцией выброса левого желудочка, получавших оптимальную медикаментозную терапию, в отношении снижения риска наступления комбинированной первичной конечной точки, включающей сердечно-сосудистую смерть или повторную госпитализацию по поводу декомпенсации сердечной недостаточности. Анализ вторичных конечных точек, включая компоненты первичной конечной точки: госпитализации по поводу декомпенсации хронической сердечной недостаточности и сердечно-сосудистую смертность, выполненный с использованием методов анализа выживаемости, не выявил статистически значимых различий между группами лечения.
Изменения показателей качества жизни по шкале Kansas City Cardiomyopathy Questionnaire (KCCQ) и уровня NT-proBNP, оценённые отдельно на 12-м и 24-м месяцах наблюдения, также не продемонстрировали статистически значимых межгрупповых различий.
Анализ безопасности, включавший оценку некоторых показателей функции почек и электролитного баланса на протяжении периода наблюдения, не выявил клинически значимых различий между группами экстракта боярышника и плацебо.
В совокупности результаты исследования не подтверждают наличие статистически значимого эффекта лечения экстрактом боярышника по сравнению с плацебо в исследуемой популяции пациентов с хронической сердечной недостаточностью со сниженной фракцией выброса левого желудочка.
>>>>>>> c229b32bdfd5dbdcf4a4f84b87311dff69db3d67
